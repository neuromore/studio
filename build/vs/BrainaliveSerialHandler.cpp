...
// constructor
BrainAliveSerialHandler::BrainAliveSerialHandler(SerialPort* port, BrainAliveDeviceBase* headset, QObject* parent) : QObject(parent)
{
	mSerialPort = port;
	mDevice = headset;
...
bool BrainAliveSerialHandler::ConfigureChannels(const Device::DeviceConfig& config)
{
	bool hasError = false;
...
bool BrainAliveSerialHandler::ReadChannelConfig(String& outConfig)
{
	outConfig.Clear();
...
// inputType = 0-7
bool BrainAliveSerialHandler::SendChannelCommands(uint32 index, bool power, uint32 gainIndex, uint32 inputType, bool bias, bool srb2, bool srb1)
{
...
bool BrainAliveSerialHandler::SendChannelPowerCommands(uint32 index, bool enable)
{
...
bool BrainAliveSerialHandler::SendStartCommand()
{
...
bool BrainAliveSerialHandler::SendStopCommand()
{
...
// reset board
bool BrainAliveSerialHandler::SendResetCommand()
{
...
bool BrainAliveSerialHandler::ReadStreamPacket(BrainAliveStreamPacket* data)
{
	...
	if (header != BrainAliveStreamPacket::GetProtocolHeader())
	{
		...
		else
		{
			...
			while (mSerialPort->GetNumBytesAvailable() > 0 && header != BrainAliveStreamPacket::GetProtocolHeader())
			{
				mSerialPort->GetChar(&header);
#ifdef CORE_DEBUG
