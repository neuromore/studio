...
#include "../../../../studio/src/Studio/Devices/DeviceHelpers.h"
...
bool BrainAliveDriver::Init()
{
	...
	mAutoDetection = new BrainAliveAutoDetection(this);
...
void BrainAliveDriver::AddDevice(BrainAliveDeviceBase* device, const char* serialPort)
{
	mDevices.Add(device);

	// create serial thread for the device (uses the serial handler)
	BrainAliveSerialThread* serialThread = new BrainAliveSerialThread(device, serialPort);
	serialThread->start();
	...
	for (uint32 i = 0; i < numPorts; ++i)
	{
		...
		else
		{
			...
			if (buff.Contains("Daisy") == true)
				reportedDeviceType = BrainAliveDaisyDevice::TYPE_ID;
			...
			if (reportedDeviceType == BrainAliveDaisyDevice::TYPE_ID && configuredDeviceType == BrainAliveDevice::TYPE_ID)
			{
		...
		BrainAliveDeviceBase* device = static_cast<BrainAliveDeviceBase*>(mDriver->CreateDevice(deviceType));
...
void BrainAliveSerialThread::run()
{
	...
	BrainAliveSerialHandler serialHandler(&serialPort, mDevice, this);
