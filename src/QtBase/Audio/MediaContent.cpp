...
// constructor
MediaContent::MediaContent(QObject* parent, WebDataCache* cache, const char* url) : QObject(parent)
{
	mMediaPlayer = new QMediaPlayer(parent);
	//connect( mMediaPlayer, SIGNAL(positionChanged(qint64)), this, SLOT(OnMediaPositionChanged(qint64)) );
...
// called when the duration change
void MediaContent::OnMediaPositionChanged(qint64 position)
{
	//LogInfo("OnMediaPositionChanged: %i", position);
}


// called when the media file reached the end etc.
