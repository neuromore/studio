...
#ifndef __NEUROMORE_SESSIONTIMENODE_H
#define __NEUROMORE_SESSIONTIMENODE_H
...
class ENGINE_API SessionTimeNode : public InputNode
{
	public:
		enum { TYPE_ID = 0x0011 };
		static const char* Uuid () { return "36de4c1e-bb6b-11e4-8dfc-aa07a5b093db"; }
		
		enum { OUTPUTPORT_VALUE = 0 };
		
		enum 
		{ 
			ATTRIB_UNIT = NUM_INPUTNODEATTRIBUTES,
			ATTRIB_SAMPLERATE
		};
		...
		const char* GetReadableType() const override							{ return "Session Time"; }
		const char* GetRuleName() const override final							{ return "NODE_SessionTime"; }
		uint32 GetPaletteCategory() const override								{ return CATEGORY_INPUT; }
		GraphObject* Clone(Graph* graph) override								{ SessionTimeNode* clone = new SessionTimeNode(graph); return clone; }
