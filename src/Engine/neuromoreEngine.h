	...
	*/
	...
	class NEUROMORE_EXPORT Callback
	{
		public:
			...
			virtual void OnLog(const char* message) = 0;															// engine log output (info/debug/warning/error), collect this only locally
			...
			virtual void OnPlayAudio(const char* url, int numLoops, double beginAt=0.0, double volume=1.0)								{}		// play the audio starting at the specified time at specified volume (normalized 0..1). Note that it must be possible to play back multiple assets at the same time, but not the same one (Use one track for each audio asset).
			virtual void OnStopAudio(const char* url)																					{}		// stop audio playback of this asset
			virtual void OnPauseAudio(const char* url, bool unPause)																	{}		// pause or unpause the audio
			virtual void OnSetAudioVolume(const char* url, double volume)																{}		
			virtual void OnSeekAudio(const char* url, int millisecs)																	{}		// seek audio file to specified position (in milliseconds)
		
			// Video
			// note: we play only one video at a time
			virtual void OnPlayVideo(const char* url, int numLoops, double beginAt=0.0, double volume=1.0)								{}		// play a video fullscreen starting at the specified time at specified volume (normalized 0..1). Ignore this event if another video is currently playing.
			virtual void OnStopVideo()																									{}		// stop the playing video (there can be only one)
			virtual void OnPauseVideo(const char* url, bool unPause)																	{}		// pause or unpause the video playback
			virtual void OnSetVideoVolume(const char* url, double volume)																{}		
			virtual void OnSeekVideo(const char* url, int millisecs)																	{}		// seek video to specified position (in milliseconds)

			// Image
			virtual void OnShowImage(const char* url)																					{}		// show the image fullscreen
			virtual void OnHideImage()																									{}		// hide the image again

			// Text
			virtual void OnShowText(const char* text, float colorR, float colorG, float colorB, float colorA)							{}		// display some text over everything. Should be aesthetically pleasing with a font that fits the text length (it can be everything from a single word to multiple sentences).
			virtual void OnHideText()																									{}		// hide the text again
			...
			// background Color
			virtual void OnSetBackgroundColor(float colorR, float colorG, float colorB, float colorA)									{}		// sets the background color (first layer, draw beneath everything)

			// 4 zone AVE
			virtual void OnSetFourZoneAVEColors(const float* colorsR, const float* colorsG, const float* colorsB, const float* colorsA)	{}		// update the eight colors of the eight-zone AVE display. Start rendering AVE display if not already.
			virtual void OnHideFourZoneAVE()																							{}		// stop rendering AVE display

			// command
			virtual void OnCommand(const char* command)																					{}

			// Background Color
			//virtual void OnSetBackgroundColor(double red, double green, double blue)													{}		// Set the background color
