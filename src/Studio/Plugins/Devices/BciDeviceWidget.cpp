...
void BciDeviceWidget::AddSensorItems(QTreeWidgetItem* parent)
{
	...
	for (uint32 i = numNeuroSensors-1; i<numSensors; ++i)
	{
		...
		bool minimalDeviceStats = false;

		if (brandingName == AntBrandingName || brandingName == StarrbaseBrandingName) {
			minimalDeviceStats = true;
		}
		...
		item->setHidden(minimalDeviceStats);
