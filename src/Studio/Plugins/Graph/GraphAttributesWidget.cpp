...
// constructor
GraphAttributesWidget::GraphAttributesWidget(QWidget *parent)
    : QScrollArea(parent) {
  ...
  connect(mPropertyTreeWidget->GetPropertyManager(),
          SIGNAL(ValueChanged(Property *)), this,
          SLOT(OnValueChanged(Property *)));
...
// event handler callback: an attribute has changed, check if it was one of the
// displayed ones
void GraphAttributesWidget::OnAttributeUpdated(Graph *graph,
                                               GraphObject *object,
                                               Attribute *attribute) {
...
// event handler callback: graph was modified; check if graph or graphobject was
// displayed
void GraphAttributesWidget::OnGraphModified(Graph *graph, GraphObject *object) {
  if (mGraphObject != NULL &&
      (mGraphObject == graph || mGraphObject == object)) {
    ForceReInit();
  }
}

void GraphAttributesWidget::InitForGraphObject(GraphObject *object,
                                               bool force) {
  if (object == NULL) {
    InitForNode(NULL);
    return;
  ...
  if (object->GetBaseType() == Graph::BASE_TYPE)
    InitForGraph(static_cast<Graph *>(object)); // classifier
  else if (object->GetBaseType() == Node::BASE_TYPE)
    InitForNode(static_cast<Node *>(object)); // classifier node
  else if (object->GetType() == StateMachine::TYPE_ID)
    InitForGraph(static_cast<StateMachine *>(object), false); // state machine
  else if (object->GetBaseType() == State::BASE_TYPE)
    InitForNode(static_cast<State *>(object)); // state
  else if (object->GetBaseType() == Connection::BASE_TYPE)
    InitForConnection(
        static_cast<Connection *>(object)); // classifier connection
  else
    InitForNode(NULL);
}
...
void GraphAttributesWidget::InitForGraph(Graph *graph, bool showName) {
  ...
  if (showName == true)
    mNameProperty =
        mPropertyTreeWidget->GetPropertyManager()->AddStringProperty(
            mParentGroupName.AsChar(), "Name", graph->GetName(), 0, isReadOnly);
  else
    mNameProperty = NULL;
...
void GraphAttributesWidget::InitForNode(Node *node, bool showName) {
  ...
  // we can't handle nodes without parents here (they always belong to a graph
  // anyways...)
  ...
  const bool isAttributeReadOnly =
      sessionRunning || nodeLocked || (!graphSettingsMode && graphNotEditable);
  ...
  if (showName == true)
    mNameProperty =
        mPropertyTreeWidget->GetPropertyManager()->AddStringProperty(
            mParentGroupName.AsChar(), "Node Name", node->GetName(), 0,
            isNameReadOnly);
  else
    mNameProperty = NULL;
...
void GraphAttributesWidget::UpdateInterface() {
  ...
  for (uint32 i = 0; i < numLinks; ++i) {
    ...
    AttributeSettings *attributeSettings =
        object->GetAttributeSettings(curLink.mAttributeIndex);
...
void GraphAttributesWidget::AddAttributes(GraphObject *object,
  ...
  for (uint32 i = 0; i < numAttributes; ++i) {
    ...
    Core::AttributeSettings *attributeSettings =
        object->GetAttributeSettings(i);

    // create the attribute and add it to the layout
    const bool isReadOnly = (attributeSettings->IsEnabled() == false) ||
                            (disableAttributes == true);
    AttributeWidget *attributeWidget =
        GetQtBaseManager()->GetAttributeWidgetFactory()->CreateAttributeWidget(
            attribute, attributeSettings, object, isReadOnly);
    Property *property = mPropertyTreeWidget->AddProperty(
        parentGroupName, attributeSettings->GetName(), attributeWidget,
        attribute, attributeSettings, false);
...
void GraphAttributesWidget::AddConditions(GraphObject *object, bool readOnly) {
  ...
  // create the add condition attribute widget and add it as property to the
  // property widget
  ButtonAttributeWidget *addConditionAttributeWidget =
      new ButtonAttributeWidget("", Array<Attribute *>(), NULL, NULL, readOnly);
  ...
  mPropertyTreeWidget->AddProperty(conditionsGroupName.AsChar(),
                                   "Add Condition", addConditionAttributeWidget,
                                   NULL, NULL);
  ...
  QtBaseManager::MakeTransparentButton(addConditionButton,
                                       "/Images/Icons/Plus.png",
                                       "Add new state transition condition.");
  connect(addConditionButton, SIGNAL(clicked()), this,
          SLOT(OnAddConditionButtonClicked()));
  ...
  for (uint32 c = 0; c < numConditions; ++c) {
    ...
    // conditionGroupName.Format( "%s.%s.%s", mParentGroupName.AsChar(),
    // groupName, conditionName.AsChar() );
    conditionGroupName.Format("%s.%s", groupName, conditionName.AsChar());

    // create the remove condition attribute widget and add it as property to
    // the property widget
    ButtonAttributeWidget *removeConditionAttributeWidget =
        new ButtonAttributeWidget("", Array<Attribute *>(), NULL, NULL,
                                  readOnly);
    mPropertyTreeWidget->AddProperty(
        conditionGroupName.AsChar(), "Remove Condition",
        removeConditionAttributeWidget, NULL, NULL);

    // get the button from the property widget and adjust it
    QPushButton *removeConditionButton =
        removeConditionAttributeWidget->GetButton();
    QtBaseManager::MakeTransparentButton(
        removeConditionButton, "/Images/Icons/Remove.png",
        "Remove condition from the transition.");
    connect(removeConditionButton, SIGNAL(clicked()), this,
            SLOT(OnRemoveCondition()));

    // add the remove button to the table, so we know this button would remove
    // what condition
...
void GraphAttributesWidget::OnAddConditionButtonClicked() {
  if (mGraphObject == NULL ||
      mGraphObject->GetType() != StateTransition::TYPE_ID)
    return;
  ...
  for (uint32 i = 0; i < numObjects; ++i) {
    ...
    QAction *addAction =
        menu.addAction(object->GetReadableType(), this, SLOT(OnAddCondition()));
    addAction->setData(object->GetType());
    // addAction->setIcon(
    // GetQtBaseManager()->FindIcon("Images/Icons/PlusFolder.png") );
  }
  ...
  menu.exec(
      addConditionButton->mapToGlobal(addConditionButton->rect().bottomLeft()));
...
void GraphAttributesWidget::OnAddCondition() {
  ...
  GraphObject *object = GetGraphObjectFactory()->CreateObjectByTypeID(
      transition->GetStateMachine(), selectedConditionType);
  if (object == NULL)
    return;
  ...
  StateTransitionCondition *condition =
      static_cast<StateTransitionCondition *>(object);
...
void GraphAttributesWidget::OnRemoveCondition() {
  ...
  StateTransition *transition =
      static_cast<StateTransition *>(transitionObject);
  transition->GetStateMachine()->RemoveCondition(
      transition, mRemoveButtonTable[index].mIndex);
...
void GraphAttributesWidget::AddActions(GraphObject *object, bool readOnly) {
  ...
  // create the add action attribute widget and add it as property to the
  // property widget
  ButtonAttributeWidget *addActionAttributeWidget =
      new ButtonAttributeWidget("", Array<Attribute *>(), NULL, NULL, readOnly);
  ...
  mPropertyTreeWidget->AddProperty(actionsGroupName.AsChar(), "Add Action",
                                   addActionAttributeWidget, NULL, NULL);

  // get the button from the property widget and adjust it
  QPushButton *addActionButton = addActionAttributeWidget->GetButton();
  QtBaseManager::MakeTransparentButton(
      addActionButton, "/Images/Icons/Plus.png", "Add new action.");
  connect(addActionButton, SIGNAL(clicked()), this,
          SLOT(OnAddEnterActionButtonClicked()));
  ...
  for (uint32 a = 0; a < numActions; ++a) {
    ...
    // actionGroupName.Format( "%s.%s.%s", mParentGroupName.AsChar(), groupName,
    // actionName.AsChar() );
    actionGroupName.Format("%s.%s", groupName, actionName.AsChar());

    // create the remove action attribute widget and add it as property to the
    // property widget
    ButtonAttributeWidget *removeActionAttributeWidget =
        new ButtonAttributeWidget("", Array<Attribute *>(), NULL, NULL,
                                  readOnly);
    mPropertyTreeWidget->AddProperty(actionGroupName.AsChar(), "Remove Action",
                                     removeActionAttributeWidget, NULL, NULL);

    // get the button from the property widget and adjust it
    QPushButton *removeActionButton = removeActionAttributeWidget->GetButton();
    QtBaseManager::MakeTransparentButton(
        removeActionButton, "/Images/Icons/Remove.png", "Remove action.");
    connect(removeActionButton, SIGNAL(clicked()), this,
            SLOT(OnRemoveEnterAction()));
  ...
  QtBaseManager::MakeTransparentButton(
      addActionButton, "/Images/Icons/Plus.png", "Add new action.");
  connect(addActionButton, SIGNAL(clicked()), this,
          SLOT(OnAddExitActionButtonClicked()));
...
void GraphAttributesWidget::OnAddActionButtonClicked(uint32 actionTypeIndex) {
  ...
  for (uint32 i = 0; i < numObjects; ++i) {
    ...
    if (actionTypeIndex == 0)
      addAction = menu.addAction(actionReadableType.AsChar(), this,
                                 SLOT(OnAddEnterAction()));
    else
      addAction = menu.addAction(actionReadableType.AsChar(), this,
                                 SLOT(OnAddExitAction()));
